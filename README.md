...
SwiftUI と MediaPlayer を中心に構成した、
...
| タブ | 概要 |
|------|------|
| **ホーム** | 統計グリッド・パーソナリティ・TOP5・トップアーティスト（期間連動）|
| **ランキング** | 楽曲 / アーティスト / アルバム別ランキング（横スワイプ切替・期間・検索付き）|
...
- **統計グリッド（2×2）**：総再生回数・総アーティスト数・総アルバム数・総再生時間をカード形式で表示
...
- **Wrapped 風ストーリー画面**（7 ページ、自動進行 10 秒/ページ）
  1. オープニング（ユーザー名表示）
  2. 総再生回数（**0 からイーズアウトカウントアップアニメーション**・再生時間換算表示）
  3. ジャンル分布
  4. TOP アーティスト
  5. TOP 楽曲
  6. 最も聴いた月
  7. パーソナリティ覚醒（**波形背景 + 拡張グローリング + ネオンデュアルシャドウ**によるドラマチック演出）

#### 音楽パーソナリティ

- 再生履歴から **12 種類のパーソナリティ**を判定
- **ヒーローカード背景に波形アニメーション**（`.screen` blendMode で発光演出）
- 各パーソナリティに固有のアイコン（SwiftUI Canvas 描画）とアニメーション
- **横スワイプ探索モード**（12 パーソナリティを全比較、下スワイプで閉じる）
- パーソナリティ一覧と詳細スコアを表示
...
- **楽曲情報カード**：再生時間・ジャンル・音源種別（CD / ストリーミング）・追加日（YYYY/MM/DD 形式）
- **ライブラリランキングバッジ**：全楽曲中の再生数順位を表示
- アーティスト名・アルバム名をタップするとそれぞれの詳細画面へ遷移

#### SNS シェア

- **シェアカード生成**（Instagram Story 1080×1920 / X 1200×630）
- **TOP アーティスト・TOP 楽曲を 10 位まで表示**（アートワーク付き）
- **パーソナリティアイコンを静止画像としてレンダリング**して埋め込み
...
- **アルバムアートワークは `アルバム名//アーティスト名` の複合キーで管理**（同名アルバムの混在防止）
- **ローカル音源（CD 取り込み）は iTunes API フォールバックをスキップ**（Apple Music 配信版のジャケ写で上書きされない）
- アーティスト画像を Deezer / iTunes Search API から自動取得
- アートワーク未収録トラックがあっても**アルバム内全トラックを走査**して埋め込み画像を探索
...
| 今週の TOP3 | Small / Medium | 今週 TOP3 楽曲 |
| 今月のサマリー | Large | 月間統計まとめ |
| 時間帯ヒートマップ | Medium | 24 時間帯別再生ヒートマップ |
...
|------|------|
| **Swift 5.9+** | 全コード |
| **SwiftUI** | UI 全体（Canvas API によるカスタムアイコン描画・波形アニメーション含む）|
...
| API | 用途 | 備考 |
|------|------|------|
| **Deezer API** | アーティスト人物画像取得 | 無料・API キー不要 |
| **iTunes Search API** | アルバムジャケット取得（フォールバック）| 無料・API キー不要 |
...
| **Service** | ビジネスロジック（認証・履歴差分計算・パーソナリティ分析・QR 生成・画像取得など）|
...
カスタム画像（手動設定）
    ↓ なければ
MPMediaItemArtwork（アルバム全トラック走査）
    ↓ なければ（ローカル音源のみのアルバムはここでスキップ）
iTunes Search API（日本語に強い）
    ↓ なければ
プレースホルダー
```

アーティスト画像は Deezer（人物画像優先）→ iTunes の順でフォールバック。
メモリキャッシュ + ディスクキャッシュで、再取得を最小化しています。

#### デザイントークン（AppTheme）

`AppTheme` enum でカラー・スペーシング・角丸・タイポグラフィ・シャドウを一元管理:

```swift
AppTheme.Colors.accent       // .pink（メインアクセント）
AppTheme.Spacing.screen      // 16pt（画面標準パディング）
AppTheme.Radius.card         // 16pt（カード角丸）
AppTheme.Typography.heroNumber // 48pt Black Rounded
```

#### フローティングタブバー（iOS 26 Liquid Glass 対応）

- **iOS 26 以上**: ネイティブ `TabView` + `.tabItem` を使用 → 自動的に Liquid Glass デザインが適用
- **iOS 17〜25**: カスタム `FloatingTabBar`（カプセル型・ブラック半透明）を `safeAreaInset` で実装
- `#available(iOS 26, *)` で分岐し、どちらの環境でも最適な見た目を提供
...
│   ├── ContentView.swift                    # ルート View / フローティングタブバー（iOS 26 Liquid Glass 対応）
│   └── AppTheme.swift                       # デザイントークン統一（Colors / Spacing / Radius / Typography）
├── Models/
│   ├── Track.swift                          # 楽曲モデル
│   ├── Artist.swift                         # アーティストモデル
│   ├── Album.swift                          # アルバムモデル
│   ├── PlayHistoryEntry.swift               # 再生履歴エントリ
│   ├── ListeningProfile.swift               # QR 相性チェック用プロフィールモデル
...
│   └── MPMediaItem+SourceClassification.swift  # CD vs Apple Music 音源判定（isLocalAsset）
├── Persistence/
│   ├── PersistenceController.swift          # Core Data Stack（CloudKit 対応、プログラマティック定義）
│   └── PlayHistoryRepository.swift          # 再生履歴クエリ（期間指定・楽曲別など）
├── Services/
│   ├── MusicAuthService.swift               # MusicKit 認証
│   ├── MusicLibraryService.swift            # MPMediaQuery 経由でライブラリ取得・アルバム複合キー生成
│   ├── PlayHistoryTracker.swift             # 差分検知・バックフィル・精度レベル管理
│   ├── PersonalityAnalysisEngine.swift      # パーソナリティ判定ロジック（12種）
│   ├── CompatibilityEngine.swift            # 音楽相性スコア計算（4 軸）
│   ├── QRCodeService.swift                  # QR エンコード / デコード / 画像生成・合成
│   ├── ArtworkService.swift                 # アートワーク管理・多段キャッシュ・ローカル音源保護
...
│   ├── HapticsManager.swift                 # ハプティック共通管理（Haptics.play()）
...
│   ├── Home/                                # ホーム画面（統計グリッド 2×2・パーソナリティ・TOP5）
│   ├── Ranking/                             # ランキング画面（横スワイプ切替・マイクロアニメーション・先頭スクロール）
│   ├── MonthlyReport/                       # 月別レポート（横スワイプ切替）
│   ├── YearlyReport/                        # 年間レポート（横スワイプ切替）
│   ├── Personality/                         # パーソナリティ詳細（波形背景・横スワイプ探索モード）
│   ├── Genre/                               # ジャンル詳細（円グラフ + アーティスト）
│   ├── TrackDetail/                         # 楽曲詳細（追加日 YYYY/MM/DD・アーティスト/アルバムリンク）
│   ├── ArtistDetail/                        # アーティスト詳細・アルバム詳細
│   ├── QR/                                  # 音楽相性チェック（スキャナー・結果カード）
│   ├── Story/                               # Wrapped 風ストーリー（7 ページ・カウントアップ・覚醒演出）
│   ├── Share/                               # シェアカード生成（10 位表示・パーソナリティ画像埋め込み）
│   ├── Settings/                            # 設定・プロフィール編集
│   ├── More/                                # more タブメニュー
│   ├── Developer/                           # 開発者デバッグ画面
│   └── Components/                          # 共通 UI コンポーネント
│       ├── PersonalityBadgeView.swift       # パーソナリティアイコン（Canvas 描画・アニメーション）
│       ├── ArtworkView.swift                # アートワーク表示（複合キーキャッシュ）
│       ├── AccuracyLevelBanner.swift        # 履歴精度レベルバナー
│       ├── GenreReportSection.swift         # ジャンル分布セクション（レポート用）
│       └── SplashView.swift                 # スプラッシュ画面（波形パルスアニメーション）
├── Widget/
...
└── Resources/
    └── MusicLibrary.xcdatamodeld/          # Core Data スキーマ（PersistenceController でプログラマティック定義）
```
...
- シェアカード用途では `ImageRenderer` で静止スナップショットとして書き出し
...
同名アルバム（例: 異なるアーティストの "Greatest Hits"）を区別するため、
アルバム ID に `"\(albumTitle)//\(artistName)"` の複合キーを使用。
ローカル音源（CD 取り込み）のアルバムは iTunes API フォールバックを実行せず、
Apple Music 配信版の別ジャケ写で上書きされることを防ぐ。
